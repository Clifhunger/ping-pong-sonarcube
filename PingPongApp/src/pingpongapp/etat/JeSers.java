...
public class JeSers implements Etat {
    ...
    @Override
    public void init() {
        LOG.log(Level.INFO, "jeSers - init");
        throw new UnsupportedOperationException(NOT_SUPPORTED);
    }

    @Override
    public void attenteAck() {
        LOG.log(Level.INFO, "jeSers - attenteAck");
        throw new UnsupportedOperationException(NOT_SUPPORTED);
    }
    ...
    public void echange() {
        ...
        if (joueur.getMonScore() == joueur.getScoreMax() || joueur.getSonScore() == joueur.getScoreMax()) {
            ...
            if (rand.nextInt(100) + 1 <= joueur.getProbaAbandon()
                    && joueur.getMonScore() <= (joueur.getSonScore() - 3)) {
                try {
                    LOG.log(Level.INFO, "\n===========================================");
                ...
                LOG.log(Level.INFO, "\n==========================================");
                LOG.log(Level.INFO,
                        "Debut de l'échange je Sert   ==  numéro du service : " + joueur.getNumeroDuService());
